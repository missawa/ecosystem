...
type
  Tfrm_licenca_old = class(Tfrm_cad_abstrato_mestre_detalhe)
    Label1: TLabel;
    lbl_cnpj_cpf: TLabel;
    Label4: TLabel;
    Label5: TLabel;
    ...
    procedure dseNewRecord(DataSet: TDataSet);
  private
...
var
  frm_licenca_old: Tfrm_licenca_old;
...
procedure Tfrm_licenca_old.dseNewRecord(DataSet: TDataSet);
begin
  inherited;
  dse.FieldByName('id_cliente').Value := qry_cliente_atividade.Parameters.ParamByName('id_cliente').Value;
  dse.FieldByName('id_atividade').Value := qry_cliente_atividade.Parameters.ParamByName('id_atividade').Value
end;

procedure Tfrm_licenca_old.dse_detalheBeforeOpen(DataSet: TDataSet);
begin
  dse_detalhe.Parameters.ParamByName('id_licenca').Value := dse.FieldByName(key_field).Value;
end;

procedure Tfrm_licenca_old.dse_detalheNewRecord(DataSet: TDataSet);
begin
  inherited;
  dse_detalhe.FieldByName('id_licenca').AsInteger := dse.FieldByName('id_licenca').AsInteger;
end;

procedure Tfrm_licenca_old.open_aux_queries;
begin
  qry_orgao.Open;
  qry_tipo.Open;
  qry_municipio.Open;
...
procedure Tfrm_licenca_old.FormCreate(Sender: TObject);
begin
...
procedure Tfrm_licenca_old.open_dataset(pk: integer; pk_detalhe: integer);
begin
